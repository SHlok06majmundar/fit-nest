...
const TrainerDetailsAdmin = ({ emails }) => {
  ...
  const handleDeleteTrainer = async (trainerId) => {
    if (window.confirm('Are you sure you want to delete this trainer?')) {
      try {
        const response = await axios.delete(`https://www.fit-nest.in/api/Trainer/delete-trainer/${trainerId}`, { withCredentials: true });
  ...
  const handleUpdateShift = async (trainerId, newShift) => {
    if (window.confirm('Are you sure you want to remove this shift?')) {
      try {
        const response = await axios.put(`https://www.fit-nest.in/api/Trainer/remove-shift/${trainerId}`, { day: newShift.day, time: newShift.time }
  ...
  const handleUpdateSalary = async (trainerId, newSalary) => {
    try {
      await axios.put(`https://www.fit-nest.in/api/Trainer/update-salary/${trainerId}`, { salary: newSalary }, { withCredentials: true });
      setTrainers((prevTrainers) =>
        prevTrainers.map((trainer) =>
  ...
  const handleToggleAvailability = async (trainerId, currentAvailability) => {
    try {
      await axios.put(`https://www.fit-nest.in/api/Trainer/update-availability/${trainerId}`, {
