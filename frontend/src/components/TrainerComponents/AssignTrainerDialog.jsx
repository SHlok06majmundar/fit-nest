...
const AssignTrainerDialog = ({ open, onClose, trainers, emails }) => {
  ...
  const fetchAssignedTrainers = async (selectedTrainer) => {
    try {
      const response = await axios.get(
        `https://www.fit-nest.in/api/Trainer/GetAssignedTrainers/${selectedTrainer}`, { withCredentials: true }
      );
  ...
  const handleApprove = async (AdminActions, trainerId, memberId, name, email, contact, startDate, endDate, extra_payment) => {
    try {
      const response = await axios.put(`https://www.fit-nest.in/api/Trainer/approve-trainer/${trainerId}/${memberId}`, { AdminActions, name, email, contact, startDate, endDate, extra_payment }
  ...
  const removeTrainerRefund = async (memberId, trainerId, paymentId, amount, userName, userEmail, startDate, endDate) => {
    if (window.confirm('Are you sure you want to remove this assigment and refund the user ?')) {
      try {
        const response = await axios.put(`https://www.fit-nest.in/api/Trainer/remove-trainer-refund/${memberId}/${trainerId}`, {
          paymentId, amount
          , userEmail: userEmail,
          userName: userName,
          startDate: startDate,
  ...
  const removeTrainer = async (memberId, trainerId) => {
    if (window.confirm('Are you sure you want to remove this request from user ?')) {
      try {
        await axios.put(
          `https://www.fit-nest.in/api/Trainer/RemoveTrainer/${memberId}/${trainerId}`, {}, { withCredentials: true }
        );
  ...
  const handleRequestToPay = async (AdminActions, trainerId, memberId, name, email, contact, startDate, endDate, extra_payment) => {
    ...
    try {
      const response = await axios.put(`https://www.fit-nest.in/api/Trainer/request-to-pay-for-user/${trainerId}/${memberId}`, { AdminActions, name, email, contact, startDate, endDate, extra_payment }
        , { withCredentials: true }
      );
      if (response.status === 200) {
        fetchAssignedTrainers(trainerId);
