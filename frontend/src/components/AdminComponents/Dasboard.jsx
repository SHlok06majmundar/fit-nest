...
const Dashboard = () => {
  ...
  const fetchTotalSales = async (startDate, endDate, type) => {
    try {
      const response = await axios.get(
        `http://16.176.121.1/api/Order/TotalSales/${startDate}/${endDate}`, { withCredentials: true }
      );
      if (type === 'monthly') {
  ...
  const fetchInactiveMembers = async () => {
    try {
      const response = await axios.get('http://16.176.121.1/api/users/inactive-members', { withCredentials: true });
  ...
  const fetchSalesByProduct = async (startDate, endDate) => {
    try {
      const response = await axios.get(`http://16.176.121.1/api/Admin/getSalesDataByProduct/${startDate}/${endDate}`
