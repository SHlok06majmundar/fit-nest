...
const ProductSalesPieChart = ({ startDate, endDate }) => {
  ...
  useEffect(() => {
    ...
    const fetchMostandLeastBoughtProducts = async (startDate, endDate) => {
      try {
        const response = await axios.get(`http://16.176.121.1/api/Order/Most-least-products/${startDate}/${endDate}`, { withCredentials: true });
    ...
    const fetchData = async (startDate, endDate) => {
      try {
        const response = await axios.get(
          `http://16.176.121.1/api/Order/products-buy-counts/${startDate}/${endDate}`, { withCredentials: true }
        );
