    ...
    try {
      ...
      for (let i = 0; i < excelData.length; i++) {
        ...
        addProductPromises.push(
          axios.post(
            'https://www.fit-nest.in/api/products/AddProduct',
 ...
 */
  const handleDeleteProduct = (id) => {
    if (window.confirm('Are you sure you want to delete this product?')) {
      axios.delete(`https://www.fit-nest.in/api/products/deleteProduct/${id}`, { withCredentials: true })
        .then(response => {
 ...
 */
  const handleIncrementProduct = async (id) => {
    if (window.confirm('Are you sure you want to increment the stock of this product?')) {
      try {
        const response = await axios.put(`https://www.fit-nest.in/api/products/incrementProduct/${id}`, {}, { withCredentials: true });
 ...
 */
  const handleDecrementProduct = async (id) => {
    if (window.confirm('Are you sure you want to decrement the stock of this product?')) {
      try {
        const response = await axios.put(`https://www.fit-nest.in/api/products/decrementProduct/${id}`, {}, { withCredentials: true });
        if (response.data.message === 'Product stock updated') {
          setProducts(prevProducts => prevProducts.map(product => {
            if (product._id === response.data.updatedProduct._id) {
              return response.data.updatedProduct;
 ...
 */
  ...
  const handleAddProductManual = async () => {
    try {
      const response = await axios.post('https://www.fit-nest.in/api/products/AddProduct', productData, { withCredentials: true });
      if (response.data) {
