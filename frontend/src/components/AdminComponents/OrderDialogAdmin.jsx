...
const OrderDialogAdmin = ({ open, onClose }) => {
  ...
  const fetchUserOrders = async () => {
    const response = await axios.post(`https://www.fit-nest.in/api/Order/AllOrders`, {}, { withCredentials: true });
  ...
  const handleSubmitDate = (orderId) => {
    ...
    if (window.confirm('Are you sure you want to update the delivery date of this order?')) {
      axios.put(`https://www.fit-nest.in/api/Order/EditDeliveryTime/${orderId}`, { deliveryDate: newDeliveryDate, userName: selectedOrder.UserName, email: selectedOrder.UserEmail }
  ...
  const handleSubmitStatus = (orderId) => {
    ...
    if (window.confirm('Are you sure you want to update the status of this order?')) {
      axios
        .put(`https://www.fit-nest.in/api/Order/EditStatus/${orderId}`, { status: newStatus, userName: selectedOrder.UserName, email: selectedOrder.UserEmail }, { withCredentials: true })
        .then((response) => {
