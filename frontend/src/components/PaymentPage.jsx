...
const PaymentPage = () => {
  ...
  const fetchProducts = async () => {
    try {
      const response = await axios.get(
        "https://www.fit-nest.in/api/products/AllProducts",
  ...
  const fetchCartProductsOfUser = async (id) => {
    if (id === null) {
      ...
      try {
        const response = await axios.get(`https://www.fit-nest.in/api/Cart/GetCarts/${id}`, { withCredentials: true });
  ...
  //         `https://www.fit-nest.in/api/Cart/DeleteFromCartAfterOrder/${Authuser._id}/${product.productId}`
  //         ,{withCredentials:true}
  //       );
      ...
      try {
        for (const product of selectedProducts) {
          const response = await axios.put(
            `https://www.fit-nest.in/api/Cart/DeleteFromCartAfterOrder-guest/${product.productId}`
            , { count: product.count }
            , { withCredentials: true }
          );
          if (response.data.message !== "Product deleted from cart successfully") {
  ...
  const handlePlaceLockOnOrder = async (selectedProducts) => {
    if (!Authuser) {
      try {
        for (const product of selectedProducts) {
          const response = await axios.put(`https://www.fit-nest.in/api/Cart/lockProductDuringPay-guest/${product.productId}`,
            { count: product.count },
            { withCredentials: true });
    ...
    try {
      for (const product of selectedProducts) {
        const response = await axios.put(`https://www.fit-nest.in/api/Cart/lockProductDuringPay/${product.productId}/${Authuser._id}`, {}, { withCredentials: true });
        if (response.data.message !== "Product locked successfully") {
          throw new Error("Failed to lock product");
        }
      }
  ...
  const handleReleaseLockDue = async (selectedProducts) => {
    if (!Authuser) {
      try {
        for (const product of selectedProducts) {
          const response = await axios.put(`https://www.fit-nest.in/api/Cart/releaseLockDueToPayFailure-guest/${product.productId}`,
            { count: product.count },
            { withCredentials: true });
    ...
    try {
      for (const product of selectedProducts) {
        const response = await axios.put(`https://www.fit-nest.in/api/Cart/releaseLockDueToPayFailure/${product.productId}/${Authuser._id}`, {}, { withCredentials: true });
        if (response.data.message !== "Product unlocked successfully") {
          throw new Error("Failed to unlock product");
        }
      }
  ...
  const handlePayment = async () => {
    ...
    try {
      const orderResponse = await axios.post(
        Authuser ? "https://www.fit-nest.in/api/Payment/create-order-multiple"
          : "https://www.fit-nest.in/api/Payment/create-order-multiple-guest",
        {
        ...
        handler: async (response) => {
          try {
            ...
            const verificationResponse = await axios.post(
              Authuser ? "https://www.fit-nest.in/api/Payment/verify-payment-for-multiple" :
                "https://www.fit-nest.in/api/Payment/verify-payment-for-multiple-guest",
              {
