...
const OrdersDialog = ({ open, onClose, userId }) => {
  ...
  const fetchUserOrders = async (userId) => {
    const response = await axios.post(`https://www.fit-nest.in/api/Order/GetAllOrders/${userId}`, {}, { withCredentials: true });
    return response.data;
  };
  const handleCancelOrder = async (orderId) => {
    if (window.confirm('Are you sure you want to cancel this order?')) {
      try {
        const response = await axios.put(`https://www.fit-nest.in/api/Order/CancelOrder/${orderId}`, {}, { withCredentials: true });
  ...
  const handleAddressUpdate = async (orderId) => {
    try {
      const response = await axios.put(`https://www.fit-nest.in/api/Order/UpdateOrderAddress/${orderId}`, {
