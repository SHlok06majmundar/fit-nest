...
const Login2 = () => {
  ...
  const checkEmail = async (email) => {
    ...
    try {
      const response = await axios.post("https://www.fit-nest.in/api/auth/check-email", { email })
  ...
  const sendOtp = async (email) => {
    try {
      if (email) {
        setLoading(true)
        await axios.post(
          "https://www.fit-nest.in/api/auth/send-otp",
          {
            email,
  ...
  const verifyOtp = async () => {
    try {
      const response = await axios.post(
        "https://www.fit-nest.in/api/auth/verify-otp",
        { email, otp },
        { withCredentials: true },
      )
  ...
  const handleSaveNewPassword = async () => {
    ...
    try {
      const response = await axios.put("https://www.fit-nest.in/api/auth/save-new-password", {
        email,
