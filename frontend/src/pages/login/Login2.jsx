...
const Login2 = () => {
  ...
  const checkEmail = async (email) => {
    ...
    try {
      const response = await axios.post("http://16.176.121.1/api/auth/check-email", { email })
  ...
  const sendOtp = async (email) => {
    try {
      if (email) {
        setLoading(true)
        await axios.post(
          "http://16.176.121.1/api/auth/send-otp",
          {
            email,
  ...
  const verifyOtp = async () => {
    try {
      const response = await axios.post(
        "http://16.176.121.1/api/auth/verify-otp",
        { email, otp },
        { withCredentials: true },
      )
  ...
  const handleSaveNewPassword = async () => {
    ...
    try {
      const response = await axios.put("http://16.176.121.1/api/auth/save-new-password", {
        email,
