...
const Home = () => {
  ...
  const handleRemoveFromCart = async (productId) => {
    try {
      const response = await axios.delete(
        `https://www.fit-nest.in/api/Cart/DeleteFromCart/${Authuser._id}/${productId}`,
      ...
      for (const item of data) {
        ...
        for (let i = 0; i < count; i++) {
          const response = await axios.put(
            `https://www.fit-nest.in/api/Cart/AddToCart-from-local-storage/${AuthUserId}/${productId}`,
            {},
            { withCredentials: true },
          )
  ...
  const handleAddToCart = async (product) => {
    if (!Authuser) {
      try {
        ...
        if (existingProductIndex !== -1) {
          ...
          const response = await axios.put(
            `https://www.fit-nest.in/api/products/decrementProduct-guest/${product._id}`,
            { cartItem },
            { withCredentials: true },
          )
    ...
    try {
      const response = await axios.put(`https://www.fit-nest.in/api/Cart/AddToCart/${Authuser._id}/${product._id}`, {})
      if (response.data.message === "Product added to cart successfully") {
