...
const MemberDashboard = () => {
  ...
  const FetchUserById= async()=>{
    ...
    try{
      setLoading(true)
      const response = await axios.get(`https://www.fit-nest.in/api/users/GetUserById/${Authuser._id}`, { withCredentials: true })
  ...
  const handleFormSubmit = async (formData) => {
    ...
    try {
      const res = await axios.post(
        `https://www.fit-nest.in/api/users/workoutDietPdfReguest/${Authuser._id}`,
        {
          age: formData?.age,
    ...
    try {
     ...
     if(!handleInputErrors(editableInfo)){
      ...
      const response = await axios.post(
        `https://www.fit-nest.in/api/users/requestToChangeInfo`,
